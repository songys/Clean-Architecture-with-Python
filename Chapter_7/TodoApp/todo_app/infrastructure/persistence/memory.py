# 인메모리 리포지토리 구현 — 테스트 및 개발 환경에 적합한 경량 저장소
# 애플리케이션 계층에서 정의한 TaskRepository, ProjectRepository 인터페이스 구현
...
class InMemoryTaskRepository(TaskRepository):
    """TaskRepository의 인메모리 구현 — 딕셔너리 기반 경량 저장소."""

    def __init__(self) -> None:
        # UUID를 키로 하는 딕셔너리에 작업 객체 저장
    ...
    def get(self, task_id: UUID) -> Task:
        ...
        Raises:
            TaskNotFoundError: 지정된 ID의 작업이 존재하지 않는 경우
        """
...
class InMemoryProjectRepository(ProjectRepository):
    """ProjectRepository의 인메모리 구현 — INBOX 자동 초기화 포함."""

    def __init__(self) -> None:
        self._projects: Dict[UUID, Project] = {}
        self._task_repo: Optional[TaskRepository] = None  # 작업 로딩용 참조
        self._initialize_inbox()  # INBOX 프로젝트 자동 생성

    def _initialize_inbox(self) -> None:
        """
        INBOX 프로젝트가 존재하지 않으면 초기화한다.
        핵심은 INBOX의 존재를 인프라스트럭처가 보장하지만,
    ...
    def _fetch_inbox(self) -> Optional[Project]:
        ...
        Returns:
            INBOX 프로젝트가 존재하면 해당 프로젝트, 그렇지 않으면 None
        """
        return next(
    ...
    def get(self, project_id: UUID) -> Project:
        ...
        Raises:
            ProjectNotFoundError: 지정된 ID의 프로젝트가 존재하지 않는 경우
        """
