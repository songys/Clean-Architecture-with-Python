...
인프라스트럭처 계층에 위치하며, 애플리케이션 계층에서 정의한
TaskRepository와 ProjectRepository 인터페이스의 파일 시스템 구현체.
도메인 객체를 JSON으로 직렬화/역직렬화하여 영속화.
"""
...
# 도메인 객체(UUID, datetime, Enum 등)를 JSON으로 변환하기 위한 커스텀 인코더
class JsonEncoder(json.JSONEncoder):
    """도메인 객체를 위한 커스텀 JSON 인코더 — UUID, datetime, Enum 지원."""
...
class FileTaskRepository(TaskRepository):
    """TaskRepository의 JSON 파일 기반 구현 — 작업 데이터를 tasks.json에 영속화."""

    def __init__(self, data_dir: Path):
        # 데이터 디렉터리 내 tasks.json 파일에 작업 저장
...
class FileProjectRepository(ProjectRepository):
    """ProjectRepository의 JSON 파일 기반 구현 — 프로젝트 데이터를 projects.json에 영속화."""

    def __init__(self, data_dir: Path):
        # 데이터 디렉터리 내 projects.json 파일에 프로젝트 저장
        self.projects_file = data_dir / "projects.json"
        self._ensure_file_exists()
        self._task_repo = None  # 작업 로딩을 위한 TaskRepository 참조 (나중에 설정)

        """
        INBOX 프로젝트 자동 초기화.
        INBOX의 존재는 인프라스트럭처가 보장하지만,
        그 동작과 규칙(삭제/완료 불가 등)은 도메인 계층에 남아 있음.
        """
    ...
    def set_task_repository(self, task_repo: TaskRepository) -> None:
        """작업 리포지토리 참조 설정 — 프로젝트의 작업 로딩에 사용"""
    ...
    def get_all(self) -> List[Project]:
        ...
        # 모든 프로젝트가 로드되고 작업 리포지토리가 설정된 후 작업을 로드
        if self._task_repo:
    ...
    def get_inbox(self) -> Project:
        ...
        inbox = self._fetch_inbox()
        if not inbox:
            raise InboxNotFoundError("Inbox 프로젝트를 찾을 수 없다")
        return inbox
    ...
    def _load_project_tasks(self, project: Project) -> None:
        ...
        try:
            ...
            # 오류를 기록하되 중단하지 않음 - 빈 작업 목록이 프로젝트 없는 것보다 낫다
