...
class ProjectController:
    """
    클린 아키텍처 패턴을 구현하는 프로젝트 관련 작업을 위한 컨트롤러.
    ...
    Attributes:
        ...
        presenter: 인터페이스를 위한 프로젝트 데이터 포맷팅을 처리
    ...
    def handle_create(self, name: str, description: str = "") -> OperationResult:
        ...
        Returns:
            ...
            - 성공: 인터페이스에 맞게 포맷된 ProjectViewModel
    ...
    def handle_complete(self, project_id: str, notes: Optional[str] = None) -> OperationResult:
        ...
        클린 아키텍처의 이점을 시연한다:
        1. 인터페이스에 구애받지 않음 - 동일한 메서드가 모든 인터페이스에서 작동
        2. 유스 케이스에 비즈니스 로직 격리
        3. 인터페이스를 위한 적절한 오류 처리와 포맷팅
