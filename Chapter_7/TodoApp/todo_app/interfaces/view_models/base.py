...
class OperationResult(Generic[T]):
    ...
    둘 다이거나 둘 다 아닐 수 없다. 이는 명시적 오류 처리와 None 검사 회피에
    도움이 된다. 또한 Mypy 및 기타 정적 타입 검사기를 통한 타입 검사를 가능하게 한다.
    ...
    def __init__(self, success: Optional[T] = None, error: Optional[ErrorViewModel] = None):
        ...
        Raises:
            ValueError: 성공과 오류 중 어느 것도 또는 둘 다 제공된 경우
        """
        if (success is None and error is None) or (success is not None and error is not None):
            raise ValueError("성공 또는 오류 중 하나만 제공해야 하며, 둘 다 제공할 수 없다")
    ...
    def success(self) -> T:
        ...
        if self._success is None:
            raise ValueError("오류 결과에서 성공 값에 접근할 수 없다")
    ...
    def error(self) -> ErrorViewModel:
        ...
        if self._error is None:
            raise ValueError("성공 결과에서 오류 값에 접근할 수 없다")
        return self._error
