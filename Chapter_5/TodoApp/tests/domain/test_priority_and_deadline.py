...
"""작업 우선순위 산출 및 마감일 값 객체 테스트."""
# Deadline 값 객체의 생성/검증/기한초과/남은시간/임박여부 테스트와
# TaskPriorityCalculator의 마감일 기반 우선순위 산출 로직 검증
...
class TestDeadline:
    ...
    def test_reject_past_deadline(self):
        """과거 마감일 생성 시 오류가 발생하는지 테스트한다."""
        past_date = datetime.now(timezone.utc) - timedelta(days=1)
        with pytest.raises(ValueError, match="마감일은 과거일 수 없습니다"):
            Deadline(past_date)

    @freeze_time("2024-01-01 12:00:00+00:00")  # freeze_time에 UTC 사용
    def test_is_overdue(self):
        """마감일이 초과되었는지 확인을 테스트한다."""
        # 지금부터 1일 후 마감일 생성 (고정된 UTC 시간 기준)
        future_date = datetime.now(timezone.utc) + timedelta(days=1)
        deadline = Deadline(future_date)
    ...
    def test_time_remaining(self):
        ...
        # 지금부터 정확히 2일 후 마감일 생성 (고정된 UTC 시간 기준)
