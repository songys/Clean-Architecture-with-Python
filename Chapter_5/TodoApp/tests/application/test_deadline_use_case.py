# 마감일 확인 유스케이스(CheckDeadlinesUseCase) 테스트
# - 빈 리포지토리, 마감일 임박/미임박, 완료된 작업 제외, 사용자 정의 임계값, 오류 처리 등 다양한 시나리오 검증
...
def test_check_deadlines_no_approaching_deadlines():
    """마감일이 임박한 작업이 없을 때 마감일 확인을 테스트한다."""
    repo = InMemoryTaskRepository()
    notifications = NotificationRecorder()
    use_case = CheckDeadlinesUseCase(repo, notifications)
...
def test_check_deadlines_approaching_deadlines():
    """마감일이 임박한 작업이 있을 때 마감일 확인을 테스트한다."""
    repo = InMemoryTaskRepository()
    notifications = NotificationRecorder()
    use_case = CheckDeadlinesUseCase(repo, notifications)
...
def test_check_deadlines_completed_tasks():
    """완료된 작업은 마감일 확인 대상에서 제외되는지 테스트한다."""
    repo = InMemoryTaskRepository()
    notifications = NotificationRecorder()
    use_case = CheckDeadlinesUseCase(repo, notifications)
...
def test_check_deadlines_handles_task_not_found():
    """마감일 확인 중 TaskNotFoundError 처리를 테스트한다."""
...
def test_check_deadlines_with_approaching_deadlines():
    ...
    # 작업 3: 기한 초과 (경고를 발생시키지 않아야 함, 다른 곳에서 처리)
    task3 = Task(title="Past Due Task", description="Test", due_date=None)  # 초기에는 마감일 없음
    # 유스 케이스 로직을 테스트하기 위해 과거 날짜에 대한 Deadline 검증을 우회해야 함
    # 실제 시나리오에서는 기한 초과 작업이 이 검사 전에 다르게 처리되거나 필터링될 수 있다.
    # 이 테스트에서는 생성 후 _due_date 속성을 수동으로 설정한다.
    task3._due_date = Deadline(datetime.now(timezone.utc) + timedelta(days=1)) # 먼저 유효한 미래 마감일 생성
    # 이제 내부 속성을 직접 수정하여 기한 초과 날짜를 시뮬레이션한다
    # 일반적으로 권장되지 않지만 테스트 조건을 격리하기 위해 필요하다.
