...
# 요청 추적을 위한 트레이스 ID 관리 모듈
# - 하나의 요청이 여러 아키텍처 계층을 거칠 때 동일한 ID로 로그를 연결
...
# ContextVar: 각 스레드/비동기 태스크별로 독립적인 값을 유지하는 변수
...
def get_trace_id() -> str:
    ...
    - 로그 포맷터에서 호출하여 모든 로그 메시지에 트레이스 ID 포함
    """
...
def set_trace_id(trace_id: Optional[str] = None) -> str:
    ...
    - 외부에서 전달된 ID가 있으면 사용 (분산 추적 시 유용)
    - 없으면 새 UUID를 생성하여 설정
    """
