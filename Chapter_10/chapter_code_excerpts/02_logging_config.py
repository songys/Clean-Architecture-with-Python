...
# - 로깅 구현 세부사항을 인프라 계층에 격리하여 다른 계층의 독립성 보장
# - JSON 구조화 로깅과 표준 텍스트 로깅 두 가지 포맷 지원
...
class JsonFormatter(logging.Formatter):
    """로그 레코드를 JSON으로 포맷하는 커스텀 포맷터.
    - 단일 책임 원칙: JSON 형식화 로직을 하나의 컴포넌트에 캡슐화
    - 모든 로그 항목에 타임스탬프, 로그 레벨, 트레이스 ID 등 필수 컨텍스트 포함
    """
    ...
    def format(self, record: logging.LogRecord) -> str:
        """로그 레코드를 JSON으로 포맷함."""
        log_data = {
            "timestamp": datetime.now(timezone.utc),  # 인코더가 datetime을 처리하도록 함
        ...
        # extra 매개변수를 사용한 로깅(예: logger.info("msg", extra={"context": {...}}))에서
        # 파이썬의 로깅 메커니즘은 이 매개변수를 LogRecord 객체에 직접 속성으로 추가한다.
        # 즉 extra={"context": {"key": "value"}}는 record.extra.context가 아닌
        # record.context로 접근 가능하게 된다.
        #
        # 이 접근 방식은 LogRecord의 내장 속성('msg', 'args', 'exc_info' 등)과의
        # 이름 충돌을 방지하기 위해 커스텀 데이터를 'context' 네임스페이스 아래에 배치한다.
        context = {}
