# 도메인 엔터티(Task) 및 유스 케이스 단위 테스트
# - 순수 도메인 로직 테스트: 프레임워크/인프라 의존 없이 검증
# - Mock을 사용한 유스 케이스 테스트: 리포지토리/알림 서비스 격리
# - 매개변수화 테스트: 다양한 입력 시나리오를 하나의 테스트로 검증
...
def test_new_task_priority():
    """순수하게 도메인 로직에 초점을 맞춘 깔끔한 테스트.
    - 외부 의존성 없이 Task 엔터티의 기본 우선순위 검증
    """
    task = Task(
        title="Test task",
