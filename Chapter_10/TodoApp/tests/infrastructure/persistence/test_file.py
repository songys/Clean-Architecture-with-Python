# 파일 기반 리포지토리 통합 테스트
# - 실제 파일 시스템(임시 디렉토리)을 사용한 영속성 검증
# - 엔터티 관계(프로젝트-작업), INBOX 자동 생성, 유스 케이스 통합 테스트
...
def test_repository_automatically_creates_inbox(tmp_path):
    ...
    # Act - 같은 디렉터리를 가리키는 새 리포지토리 인스턴스 생성
    new_repo = FileProjectRepository(tmp_path)

    # Assert - 새 인스턴스가 같은 Inbox를 유지
