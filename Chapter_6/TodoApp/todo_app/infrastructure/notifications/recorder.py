"""
인터페이스 어댑터 패턴을 설명하기 위한 간단한 알림 구현.

프레임워크 및 드라이버 계층의 사전 미리보기이다.

이 모듈은 실제 알림 서비스의 복잡성 없이 클린 아키텍처 원칙을
보여주기 위한 알림 인터페이스의 기본 구현을 제공한다. 다음을 보여준다:
- 애플리케이션 계층의 알림 인터페이스가 어떻게 구현되는지
- 알림 관심사가 비즈니스 로직과 어떻게 분리되는지
- 알림 이벤트의 기본 처리

이 구현은 7장에서 실제 알림 서비스로 대체되며,
클린 아키텍처가 핵심 기능을 유지하면서 알림 메커니즘을 교체하는
방법을 보여준다.
"""
...
class NotificationRecorder(NotificationPort):
    """
    인터페이스 어댑터 개념을 설명하기 위한 간단한 알림 구현.

    이 클래스는 클린 아키텍처에서 알림 게이트웨이가 어떻게 작동하는지 보여준다:
    - 애플리케이션 계층에서 정의한 인터페이스를 구현한다
    - 알림 세부 사항을 캡슐화한다 (이 경우 간단한 출력)
    - 알림과 비즈니스 로직 간의 분리를 유지한다

    단순화되어 있지만, 이 구현은 실제 알림 서비스 구현에서
    사용될 패턴을 확립한다.
    """

    def __init__(self) -> None:
        self.completed_tasks = []  # 완료된 작업 ID 기록 목록
        self.high_priority_tasks = []  # 높은 우선순위로 설정된 작업 ID 기록 목록
        self.deadline_warnings = []  # 마감일 임박 알림 기록 목록 (작업 ID, 남은 일수)
