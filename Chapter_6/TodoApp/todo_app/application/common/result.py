...
이러한 타입은 모든 유스 케이스에서 성공과 실패 사례를 일관되게 처리하는 방법을 제공한다.
"""
...
T = TypeVar('T')  # Result의 성공 값 타입을 위한 제네릭 타입 변수

# 애플리케이션 계층에서 발생 가능한 오류 유형을 정의하는 열거형
# - 유스케이스 실행 결과의 오류를 분류하여 외부 계층에서 적절히 처리할 수 있게 함
class ErrorCode(Enum):
    """애플리케이션 계층에서 사용 가능한 오류 코드 열거형"""

    NOT_FOUND = "NOT_FOUND"  # 요청한 엔터티를 찾을 수 없는 경우
    ...
    CONFLICT = "CONFLICT"  # 데이터 충돌


@dataclass(frozen=True)
class Error:
    """
    유스 케이스 실행 중 발생한 오류를 표현한다.

    이 클래스는 애플리케이션 계층 전반에서 오류를 표준화된 방식으로 표현하며,
    오류의 구체적인 타입(ErrorCode)과 추가 컨텍스트를 포함한다.
    ...
    # 오류 유형별 팩토리 메서드 - 일관된 오류 생성을 위한 편의 메서드
    @classmethod
...
class Result(Generic[T]):
    """
    유스 케이스 실행 결과를 Either 타입으로 표현한다.

    이 클래스는 작업의 결과를 캡슐화하며, T 타입의 값을 포함하는 성공이거나
    Error를 포함하는 실패 중 하나이다. 한 번에 하나의 상태만 존재할 수 있도록
    강제하여, 작업 결과를 처리하는 명확하고 타입 안전한 방법을 제공한다.

    Attributes:
        _value: 작업이 성공한 경우의 성공 값
        _error: 작업이 실패한 경우의 오류 정보
    ...
    def __post_init__(self):
        if (self._value is None and self._error is None) or \
           (self._value is not None and self._error is not None):
            raise ValueError("값 또는 오류 중 하나만 제공해야 하며, 둘 다 또는 둘 다 아닌 경우는 허용되지 않는다")

    @property
    def is_success(self) -> bool:
        """결과가 성공적인 작업을 나타내는지 확인한다."""
        return self._value is not None

    @property
    def value(self) -> T:
        """성공 값을 가져온다. 결과가 오류인 경우 ValueError를 발생시킨다."""
        if self._value is None:
            raise ValueError("오류 결과에서는 값에 접근할 수 없다")
        return self._value

    @property
    def error(self) -> Error:
        """오류 값을 가져온다. 결과가 성공인 경우 ValueError를 발생시킨다."""
        if self._error is None:
            raise ValueError("성공 결과에서는 오류에 접근할 수 없다")
        return self._error
