...
# 프레젠터와 뷰 사이에서 데이터를 전달하는 뷰 모델
# - 프레젠터가 도메인 데이터를 화면에 바로 표시할 수 있는 형태로 미리 변환하여 담는 객체
# - frozen=True로 불변성 보장 - 뷰가 데이터를 실수로 변경하는 것을 방지
# - 모든 필드가 문자열이므로 뷰(UI)에서 추가 변환 없이 바로 사용 가능
...
class TaskViewModel:
    ...
    due_date_display: Optional[str]  # 화면 표시용으로 미리 형식화되어 있는 마감일 (예: "Due: 2024-01-21")
    project_display: Optional[str]  # 화면 표시용 프로젝트 컨텍스트
    completion_info: Optional[str]  # 화면 표시용 완료 정보 (예: "Completed on 2024-01-20 00:00")
