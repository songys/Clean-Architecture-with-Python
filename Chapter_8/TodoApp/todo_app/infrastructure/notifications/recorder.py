...
이것은 프레임워크 및 드라이버 계층에 대한 선제적 뷰이다.

이 모듈은 실제 알림 서비스의 복잡성 없이 클린 아키텍처 원칙을 시연하기 위한
알림 인터페이스의 기본 구현을 제공한다. 다음을 보여준다:
- 애플리케이션 계층의 알림 인터페이스가 어떻게 구현되는지
- 알림 관심사가 비즈니스 로직과 어떻게 분리되는지
- 알림 이벤트의 기본 처리

이 구현은 7장에서 실제 알림 서비스로 교체될 것이며,
클린 아키텍처가 핵심 기능을 유지하면서 알림 메커니즘을
교체할 수 있게 하는 방식을 보여준다.
"""
