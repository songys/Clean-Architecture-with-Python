...
# 컴포지션 루트(Composition Root): 모든 의존성을 조립하는 진입점
# 클린 아키텍처에서 의존성 주입은 이 팩토리 함수에서 한 곳에서만 수행
...
# 애플리케이션 컨테이너: 저장소, 유스케이스, 컨트롤러를 한데 묶어 관리
# __post_init__에서 유스케이스와 컨트롤러 간의 의존성 연결(Wiring) 수행
@dataclass
class Application:
