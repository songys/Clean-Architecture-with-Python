...
# 도메인 객체(UUID, datetime, Enum)를 JSON 직렬화 가능하게 변환하는 인코더
...
# 인프라 계층: TaskRepository 인터페이스의 JSON 파일 기반 구체 구현
# 통합 테스트에서 tmp_path와 함께 사용하여 실제 영속성 동작 검증
...
# ProjectRepository의 파일 기반 구현
# 초기화 시 INBOX 프로젝트 존재를 자동으로 보장하는 인프라 로직 포함
class FileProjectRepository(ProjectRepository):
    ...
    def _load_project_tasks(self, project: Project) -> None:
        ...
        try:
            ...
            # 오류를 기록하되 중단하지 않음 - 빈 작업 목록이 프로젝트 없음보다 나음
