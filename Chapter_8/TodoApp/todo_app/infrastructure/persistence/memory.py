...
# 인메모리 구현: 딕셔너리를 사용한 TaskRepository의 경량 구현
# 단위 테스트나 개발 환경에서 파일 I/O 없이 빠르게 동작
...
# 인메모리 프로젝트 저장소: 초기화 시 INBOX 프로젝트 자동 생성 보장
class InMemoryProjectRepository(ProjectRepository):
    ...
    def _fetch_inbox(self) -> Optional[Project]:
        """
        리포지토리에서 INBOX 프로젝트를 조회한다.

        Returns:
            INBOX 프로젝트가 존재하면 해당 프로젝트, 없으면 None
        """
        return next(
