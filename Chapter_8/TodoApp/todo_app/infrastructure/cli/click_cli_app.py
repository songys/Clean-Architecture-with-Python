...
# Click 프레임워크 기반 CLI 구현: 프레임워크 & 드라이버 계층
# Application 컨테이너의 컨트롤러를 호출하여 사용자 입력 처리
class ClickCli:
    def __init__(self, app: Application):
        self.app = app
        self.current_projects = []  # 표시용 캐시된 프로젝트 목록
    ...
    def _handle_project_menu(self, project: ProjectViewModel) -> None:
        ...
        while True:
            ...
            if project.project_type != "INBOX":
                choice = click.prompt("작업을 선택하세요", type=str, default="3")
                if choice == "1":
                ...
                choice = click.prompt("작업을 선택하세요", type=str, default="2")
                if choice == "1":
                    self._add_task_to_project(project)
                elif choice == "2":
                    break
    ...
    def _display_task_menu(self, task_id: str) -> None:
        ...
        while True:
            ...
            if task.completion_info:
                click.echo(f"완료 정보:  {task.completion_info}")
            click.echo("=" * 40)
            ...
            choice = click.prompt("작업을 선택하세요", type=str, default="")
    ...
    def _complete_task(self, task_id: str) -> None:
        """작업을 완료한다."""
        notes = click.prompt("완료 노트 (선택사항)", type=str, default="")
