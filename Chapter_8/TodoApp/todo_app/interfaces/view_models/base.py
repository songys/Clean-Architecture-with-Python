...
T = TypeVar("T")  # 제네릭 타입 변수: 성공 결과의 타입을 유연하게 지정


# 오류 뷰 모델: UI에 표시할 오류 메시지와 코드를 담은 불변 객체
...
# 인터페이스 계층의 Either 타입: 컨트롤러가 반환하는 성공/실패 결과
# 애플리케이션 계층의 Result와 유사하지만, UI용 뷰 모델을 담는 구조
@dataclass
class OperationResult(Generic[T]):
    """값으로 성공하거나 오류로 실패할 수 있는 작업의 결과를 나타낸다.

    이 클래스는 Either 패턴을 구현하며, 결과는 성공 또는 실패 중 하나만 될 수 있고,
    둘 다이거나 둘 다 아닌 경우는 없다. 이는 명시적 오류 처리와
    None 검사 회피에 도움이 된다. 또한 Mypy 및 기타 정적 타입 검사기를 통한
    타입 검사를 가능하게 한다.
    ...
    def __init__(self, success: Optional[T] = None, error: Optional[ErrorViewModel] = None):
        ...
        Raises:
            ValueError: 성공과 오류가 모두 제공되지 않았거나 모두 제공된 경우
        """
        if (success is None and error is None) or (success is not None and error is not None):
            raise ValueError("성공 또는 오류 중 하나만 제공해야 하며, 둘 다 제공할 수 없습니다")
    ...
    def success(self) -> T:
        ...
        Raises:
            ValueError: 오류 결과에서 성공 값에 접근하려는 경우
        """
    ...
    def error(self) -> ErrorViewModel:
        ...
        Raises:
            ValueError: 성공 결과에서 오류 값에 접근하려는 경우
        """
