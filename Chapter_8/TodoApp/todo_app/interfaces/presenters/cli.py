...
# CLI용 프레젠터 구현: DTO를 CLI 표시에 적합한 뷰 모델로 변환
# 프레젠터 테스트에서 날짜 포맷, 우선순위 표시 등의 변환 로직을 검증
class CliTaskPresenter(TaskPresenter):
    """CLI 전용 작업 프레젠터."""

    def present_task(self, task_response: TaskResponse) -> TaskViewModel:
        """CLI 표시를 위해 작업을 포맷팅한다."""
    ...
    def _format_completion_info(
        ...
        """메모가 있는 경우 포함하여 완료 정보를 포맷팅한다."""
        if not completion_date:
            return "미완료"

        base_info = f"완료일: {completion_date.strftime('%Y-%m-%d %H:%M')}"
        if completion_notes:
            return f"{base_info} - {completion_notes}"
        return base_info

    def _format_priority(self, priority: Priority) -> str:
        """CLI 표시를 위해 우선순위를 포맷팅한다."""
        display_map = {
            Priority.LOW: "Minor",
...
# CLI용 프로젝트 프레젠터: 프로젝트 DTO를 CLI 표시용 뷰 모델로 변환
class CliProjectPresenter(ProjectPresenter):
    ...
    def _format_completion_info(self, completion_date: Optional[datetime]) -> str:
        """CLI 표시를 위해 완료 정보를 포맷팅한다."""
        if completion_date:
            return f"완료일: {completion_date.strftime('%Y-%m-%d %H:%M')}"
        return "미완료"
