...
# 애플리케이션 계층에서 발생 가능한 오류 유형 분류
class ErrorCode(Enum):
    """애플리케이션 계층에서 발생 가능한 오류 코드의 열거형."""
...
class Error:
    ...
    오류의 구체적인 유형(ErrorCode로)과 추가적인 컨텍스트를 포함한다.
...
# Either 패턴 구현체 - 유스 케이스의 실행 결과를 성공/실패로 표현
# 성공 시 T 타입 값, 실패 시 Error 객체를 포함 (동시에 둘 다 가질 수 없음)
@dataclass(frozen=True)
class Result(Generic[T]):
    """
    유스 케이스 실행 결과를 Either 타입으로 나타낸다.

    이 클래스는 연산의 결과를 캡슐화하며, T 타입의 값을 포함하는 성공이거나
    Error를 포함하는 실패 중 하나이다. 한 번에 하나의 상태만 존재할 수 있도록
    강제하여, 연산 결과를 처리하는 명확하고 타입 안전한 방법을 제공한다.

    Attributes:
        _value: 성공 시 연산의 성공 값.
        _error: 실패 시 오류 정보.
    ...
    def __post_init__(self):
        if (self._value is None and self._error is None) or \
           (self._value is not None and self._error is not None):
            raise ValueError("값 또는 오류 중 하나만 제공해야 하며, 둘 다 제공하거나 둘 다 없을 수 없음")
    ...
    def value(self) -> T:
        ...
        if self._value is None:
            raise ValueError("오류 결과에서 값에 접근할 수 없음")
        return self._value
    ...
    def error(self) -> Error:
        ...
        if self._error is None:
            raise ValueError("성공 결과에서 오류에 접근할 수 없음")
        return self._error
