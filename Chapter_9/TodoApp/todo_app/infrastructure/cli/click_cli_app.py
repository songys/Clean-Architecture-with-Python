"""
프로젝트 액션이 포함된 향상된 Click 기반 CLI 구현.
클린 아키텍처의 가장 바깥쪽 계층(프레임워크/드라이버) -
Click 프레임워크를 사용하여 사용자 입력을 수집하고 컨트롤러에 전달
"""
...
# Click 프레임워크 기반 CLI 어댑터
# Flask 라우트(routes.py)와 동일한 역할 - 사용자 입력을 컨트롤러에 전달하는 어댑터
class ClickCli:
    def __init__(self, app: Application):
        # Application 컨테이너를 통해 컨트롤러에 접근
        self.app = app
        self.current_projects = []  # 표시용 프로젝트 목록 캐시

    def run(self) -> int:
        """Click CLI 애플리케이션 실행을 위한 진입점"""
        try:
            while True:
    ...
    def _display_projects(self) -> None:
        """모든 프로젝트와 해당 작업들을 표시한다."""
        click.clear()
        click.echo("\n프로젝트: [새 프로젝트 생성하려면 'np' 입력]")
    ...
    def _handle_project_menu(self, project: ProjectViewModel) -> None:
        ...
        while True:
            ...
            if project.project_type != "INBOX":
                choice = click.prompt("액션을 선택하세요", type=str, default="3")
                if choice == "1":
                ...
                choice = click.prompt("액션을 선택하세요", type=str, default="2")
                if choice == "1":
                    self._add_task_to_project(project)
                elif choice == "2":
                    break

    # CLI 작업 생성 - 웹의 new_task 라우트와 대칭 구조
    # Click의 prompt로 입력을 수집하고, 동일한 컨트롤러의 handle_create 호출
    ...
    def _handle_selection(self) -> None:
        ...
        try:
            ...
            click.secho(
                "잘못된 선택 형식입니다. 프로젝트는 '1', 작업은 '1.a' 형식으로 입력하세요.",
                fg="red",
                err=True,
            )
    ...
    def _display_task_menu(self, task_id: str) -> None:
        ...
        while True:
            ...
            choice = click.prompt("액션을 선택하세요", type=str, default="")
    ...
    def _complete_task(self, task_id: str) -> None:
        """작업을 완료한다."""
        notes = click.prompt("완료 메모 (선택사항)", type=str, default="")
