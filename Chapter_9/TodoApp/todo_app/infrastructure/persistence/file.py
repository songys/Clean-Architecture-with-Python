"""
JSON 파일 기반 리포지토리 구현.
메모리 구현체(memory.py)와 동일한 추상 인터페이스를 구현하되,
데이터를 JSON 파일로 영속화하여 애플리케이션 재시작 후에도 유지
"""
...
# 도메인 객체(UUID, datetime, Enum)를 JSON 직렬화하기 위한 커스텀 인코더
...
# JSON 파일 기반 TaskRepository 구현체 - 데이터를 tasks.json에 저장
...
# JSON 파일 기반 ProjectRepository 구현체 - 데이터를 projects.json에 저장
...
class FileProjectRepository(ProjectRepository):
    ...
    def _load_project_tasks(self, project: Project) -> None:
        ...
        try:
            ...
            # 오류를 로깅하되 충돌하지 않음 - 빈 작업 목록이 프로젝트 없는 것보다 나음
            print(f"프로젝트 {project.id}의 작업 로드 중 오류 발생: {str(e)}")
