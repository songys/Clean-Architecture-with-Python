...
class ProjectController:
    ...
    이 컨트롤러는 주요 클린 아키텍처 원칙을 시연한다:
    - 컨트롤러는 인터페이스 어댑터 계층에 존재
    - 유스 케이스에 대해 내부 방향으로 의존 (의존성 규칙)
    - 계층 간 데이터 변환을 처리
    - 비즈니스 로직과 인터페이스 관심사를 분리

    이 클린 아키텍처 구현의 이점:
    - 비즈니스 로직이 유스 케이스에서 보호됨
    - 핵심 로직을 변경하지 않고 새 인터페이스 추가 가능
    - 의존성 주입을 통해 테스트가 간소화됨
    - 프레젠테이션 관심사가 적절히 분리됨

    Attributes:
        create_use_case: 프로젝트 생성을 위한 유스 케이스
        complete_use_case: 프로젝트 완료를 위한 유스 케이스
        presenter: 인터페이스를 위한 프로젝트 데이터 포맷팅 담당
        update_use_case: 프로젝트 업데이트를 위한 유스 케이스
    """
    ...
    def handle_create(self, name: str, description: str = "") -> OperationResult:
        ...
        1. 기본 타입을 받아들임 (인터페이스에 구애받지 않음)
        2. 데이터를 유스 케이스 형식으로 변환
        3. 유스 케이스를 통해 비즈니스 로직 실행
        4. 응답을 인터페이스에 적합한 형식으로 변환
    ...
    def handle_complete(self, project_id: str, notes: Optional[str] = None) -> OperationResult:
        ...
        클린 아키텍처의 이점을 시연한다:
        1. 인터페이스 무관 - 동일한 메서드가 모든 인터페이스에서 작동
        2. 유스 케이스에서의 비즈니스 로직 분리
        3. 인터페이스를 위한 적절한 오류 처리 및 포맷팅
    ...
    def handle_get(self, project_id: str) -> OperationResult[ProjectViewModel]:
        ...
        Returns:
            다음 중 하나를 포함하는 OperationResult:
            - 성공: 프로젝트 상세가 포함된 ProjectViewModel
            - 실패: 오류 정보
        """
        try:
