...
# ──────────────────────────────────────────────────────────────
# 인터페이스 어댑터 계층 - 주문 컨트롤러
# 웹 요청(Dict) ↔ 도메인 요청(CreateOrderRequest) 사이의 양방향 변환 담당
# Flask 등 특정 프레임워크에 의존하지 않는 순수 Python 클래스
# ──────────────────────────────────────────────────────────────
...
class OrderController:
    ...
    def handle_create_order(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            # 웹 요청(Dict)을 도메인 요청 형식으로 변환 (입력 어댑터)
            ...
            # 도메인 응답을 웹 응답 형식(Dict)으로 변환 (출력 어댑터)
            return {
            ...
            raise Exception(f"오류가 발생했습니다: {str(e)}")
